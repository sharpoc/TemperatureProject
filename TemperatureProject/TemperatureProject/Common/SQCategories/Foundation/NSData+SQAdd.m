...
- (NSString *)sq_hmacSHA224StringWithKey:(NSString *)key {
    return [self sq_hmacStringUsingAlg:kCCHmacAlgSHA224 withKey:key];
}

- (NSData *)sq_hmacSHA224DataWithKey:(NSData *)key {
    return [self sq_hmacDataUsingAlg:kCCHmacAlgSHA224 withKey:key];
}
...
- (NSString *)sq_hmacSHA384StringWithKey:(NSString *)key {
    return [self sq_hmacStringUsingAlg:kCCHmacAlgSHA384 withKey:key];
}

- (NSData *)sq_hmacSHA384DataWithKey:(NSData *)key {
    return [self sq_hmacDataUsingAlg:kCCHmacAlgSHA384 withKey:key];
}
...
- (NSData *)sq_aes256EncryptWithKey:(NSData *)key iv:(NSData *)iv {
...
- (NSData *)sq_aes256DecryptWithkey:(NSData *)key iv:(NSData *)iv {
    if (key.length != 16 && key.length != 24 && key.length != 32) {
        return nil;
    }
    if (iv.length != 16 && iv.length != 0) {
...
+ (NSData *)sq_dataWithHexString:(NSString *)hexStr {
...
+ (NSData *)sq_dataWithBase64EncodedString:(NSString *)base64EncodedString {
...
- (id)sq_jsonValueDecoded {
    ...
    if (error) {
        SQCGLog(@"jsonValueDecoded error:%@", error);
    }
    return value;
}
