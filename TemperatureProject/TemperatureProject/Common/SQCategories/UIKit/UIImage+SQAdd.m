...
@implementation UIImage (SQAdd)

+ (UIImage *)sq_imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
...
+ (UIImage *)sq_imageWithPDF:(id)dataOrPath {
    return [self _yy_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)sq_imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self _yy_imageWithPDF:dataOrPath resize:YES size:size];
}

+ (UIImage *)sq_imageWithEmoji:(NSString *)emoji size:(CGFloat)size {
...
+ (UIImage *)sq_imageWithColor:(UIColor *)color size:(CGSize)size {
...
- (UIImage *)sq_imageByResizeToSize:(CGSize)size {
...
- (UIImage *)sq_imageByResizeToSize:(CGSize)size contentMode:(UIViewContentMode)contentMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    UIGraphicsBeginImageContextWithOptions(size, NO, self.scale);
    [self sq_drawInRect:CGRectMake(0, 0, size.width, size.height) withContentMode:contentMode clipsToBounds:NO];
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return image;
}

- (UIImage *)sq_imageByCropToRect:(CGRect)rect {
...
- (UIImage *)sq_imageByInsetEdge:(UIEdgeInsets)insets withColor:(UIColor *)color {
...
- (UIImage *)sq_imageByRoundCornerRadius:(CGFloat)radius {
    return [self sq_imageByRoundCornerRadius:radius borderWidth:0 borderColor:nil];
}

- (UIImage *)sq_imageByRoundCornerRadius:(CGFloat)radius
                          borderWidth:(CGFloat)borderWidth
                          borderColor:(UIColor *)borderColor {
    return [self sq_imageByRoundCornerRadius:radius
...
- (UIImage *)sq_imageByRotate:(CGFloat)radians fitSize:(BOOL)fitSize {
...
- (UIImage *)sq_imageByRotateLeft90 {
    return [self sq_imageByRotate:DegreesToRadians(90) fitSize:YES];
}

- (UIImage *)sq_imageByRotateRight90 {
    return [self sq_imageByRotate:DegreesToRadians(-90) fitSize:YES];
}

- (UIImage *)sq_imageByRotate180 {
    return [self _yy_flipHorizontal:YES vertical:YES];
}

- (UIImage *)sq_imageByFlipVertical {
    return [self _yy_flipHorizontal:NO vertical:YES];
}

- (UIImage *)sq_imageByFlipHorizontal {
    return [self _yy_flipHorizontal:YES vertical:NO];
}

- (UIImage *)sq_imageByTintColor:(UIColor *)color {
    UIGraphicsBeginImageContextWithOptions(self.size, NO, self.scale);
    CGRect rect = CGRectMake(0, 0, self.size.width, self.size.height);
    [color set];
    UIRectFill(rect);
...
- (UIImage *)sq_imageByGrayscale {
    return [self sq_imageByBlurRadius:0 tintColor:nil tintMode:0 saturation:0 maskImage:nil];
}

- (UIImage *)sq_imageByBlurSoft {
    return [self sq_imageByBlurRadius:60 tintColor:[UIColor colorWithWhite:0.84 alpha:0.36] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}

- (UIImage *)sq_imageByBlurLight {
    return [self sq_imageByBlurRadius:60 tintColor:[UIColor colorWithWhite:1.0 alpha:0.3] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}

- (UIImage *)sq_imageByBlurExtraLight {
    return [self sq_imageByBlurRadius:40 tintColor:[UIColor colorWithWhite:0.97 alpha:0.82] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}

- (UIImage *)sq_imageByBlurDark {
    return [self sq_imageByBlurRadius:40 tintColor:[UIColor colorWithWhite:0.11 alpha:0.73] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}

- (UIImage *)sq_imageByBlurWithTint:(UIColor *)tintColor {
    ...
    return [self sq_imageByBlurRadius:20 tintColor:effectColor tintMode:kCGBlendModeNormal saturation:-1.0 maskImage:nil];
}

- (UIImage *)sq_imageByBlurRadius:(CGFloat)blurRadius
                     tintColor:(UIColor *)tintColor
                      tintMode:(CGBlendMode)tintBlendMode
                    saturation:(CGFloat)saturation
                     maskImage:(UIImage *)maskImage {
    if (self.size.width < 1 || self.size.height < 1) {
        SQCGLog(@"UIImage+YYAdd error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        SQCGLog(@"UIImage+YYAdd error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        SQCGLog(@"UIImage+YYAdd error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (hasNewFunc) {
        ...
        if (err != kvImageNoError) {
            SQCGLog(@"UIImage+YYAdd error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
        err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (err != kvImageNoError) {
            SQCGLog(@"UIImage+YYAdd error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
    } else {
...
//图片局部拉伸
- (UIImage *)sq_stretchableImage:(NSInteger)leftCapWidth topCapHeight:(NSInteger)topCapHeight {
...
- (UIImage *)sq_applyLightEffect:(UIColor*)color{
    ...
    return [self sq_applyBlurWithRadius:10 tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (UIImage*)sq_gaussBlur:(CGFloat)blurLevel {
    ...
    // convolution
    error = vImageConvolve_ARGB8888(&inBuffer,
                                    &outBuffer,
                                    NULL,
                                    0,
                                    0,
                                    ...
                                    NULL,
                                    kvImageEdgeExtend);
    error = vImageConvolve_ARGB8888(&outBuffer,
                                    &inBuffer,
                                    NULL,
                                    0,
                                    0,
    ...
    if (error) {
        SQCGLog(@"error from convolution %ld", error);
    }
...
- (NSData *)compressWithMaxLength:(NSUInteger)maxLength {
    ...
    //SQCGLog(@"Before compressing quality, image size = %ld KB",data.length/1024);
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        //SQCGLog(@"Compression = %.1f", compression);
        //SQCGLog(@"In compressing quality loop, image size = %ld KB", data.length / 1024);
    ...
    //SQCGLog(@"After compressing quality, image size = %ld KB", data.length / 1024);
    if (data.length < maxLength) return data;
    ...
    while (data.length > maxLength && data.length != lastDataLength) {
        ...
        //SQCGLog(@"In compressing size loop, image size = %ld KB", data.length / 1024);
    }
    //SQCGLog(@"After compressing size loop, image size = %ld KB", data.length / 1024);
    return data;
}
@end
