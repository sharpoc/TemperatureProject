...
- (CGRect)sq_currentBounds {
    return [self sq_boundsForOrientation:[[UIApplication sharedApplication] statusBarOrientation]];
}
#endif
...
- (CGSize)sizeInPixel {
    ...
    if ([[UIScreen mainScreen] isEqual:self]) {
        ...
        if ([model hasPrefix:@"iPhone"]) {
            ...
            if ([model isEqualToString:@"iPhone10,2"]) return
                CGSizeMake(1080, 1920);
            if ([model isEqualToString:@"iPhone10,5"]) return
                CGSizeMake(1080, 1920);
        }
