...
+ (UIFont *)sq_fontWithCGFont:(CGFontRef)CGFont size:(CGFloat)size {
...
+ (BOOL)sq_loadFontFromPath:(NSString *)path {
    ...
    if (!suc) {
        SQCGLog(@"Failed to load font: %@", error);
    }
    return suc;
}
...
+ (UIFont *)sq_loadFontFromData:(NSData *)data {
...
+ (BOOL)sq_unloadFontFromData:(UIFont *)font {
    ...
    CFRelease(fontRef);
    if (!suc) SQCGLog(@"%@", errorRef);
    return suc;
}

+ (NSData *)sq_dataFromFont:(UIFont *)font {
    CGFontRef cgFont = font.CGFontRef;
    NSData *data = [self sq_dataFromCGFont:cgFont];
...
+ (NSData *)sq_dataFromCGFont:(CGFontRef)cgFont {
    if (!cgFont) return nil;
